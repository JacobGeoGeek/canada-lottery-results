...
def extract_lotto_numbers_by_year(year: int) -> list[Numbers]:
...
def extract_lotto_result_by_date_and_region(
    date: datetime.date,
...
def _get_stats_summary(summary_contents: list[ResultSet], total_prize_fund: float) -> Summary:
    ...
    winning_ratio: str  = summary_contents[2].find(class_=stat_class).text.split(" ")[0]
    ...
    return Summary(
        ...
        winning_ratio=None if not winning_ratio else int(winning_ratio) / 100,
